...
export default function VerifikasiPengajuanDana({ menunggu = [], diteruskan = [] }) {
    const handleSetuju = (id) => {
        if (confirm('Teruskan laporan pengajuan dana ini ke Manager Accounting?')) {
            router.put(route('staff.verifikasi-pengajuan-dana.setuju', id));
        }
    };
    ...
    const handleRevisi = (id) => {
        ...
        if (!catatan) return;
        router.put(route('staff.verifikasi-pengajuan-dana.revisi', id), { catatan });
    };

    return (
        <AuthenticatedLayout header="Verifikasi Pengajuan Dana">
            <Head title="Verifikasi Pengajuan Dana - Staff Accounting" />

            <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900">Menunggu Verifikasi Kelengkapan</h3>
            ...
            <div className="space-y-4 mb-10">
                {menunggu.length === 0 ? (
                    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 text-center text-gray-500">
                        Tidak ada laporan pengajuan dana yang menunggu verifikasi.
                    </div>
                ) : (
                    menunggu.map((fp) => (
            ...
            <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900">Sudah Diteruskan ke Manager</h3>
                <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2.5 py-0.5 rounded-full">{diteruskan.length}</span>
            </div>
                            ...
                            {diteruskan.length === 0 ? (
                                <tr>
                                    <td colSpan="3" className="px-6 py-10 text-center text-gray-500 text-sm">
                                        Belum ada pengajuan yang diteruskan.
                                    </td>
                                </tr>
                            ) : (
                                diteruskan.map((fp) => (
                                    <tr key={fp.id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {Number(fp.amount).toLocaleString('id-ID')}</td>
                                        <td className="px-6 py-4 whitespace-nowrap"><Badge status={fp.status} /></td>
                                    </tr>
                                ))
                            )}
                        </tbody>
